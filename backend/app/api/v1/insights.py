...
async def get_top_dishes(
    ...
    - 推荐改进和淘汰建议
    """
    dishes = await insights_service.get_top_dishes(db, current_user, period)
    return success(
...
async def get_three_good_three_bad(
    ...
    - 三项最需要改进的不足
    """
    report = await insights_service.get_three_good_three_bad(db, current_user, period)
...
async def get_dish_elimination(
    ...
    - 淘汰原因和改进建议
    """
    suggestions = await insights_service.get_dish_elimination(db, current_user)
    return success(
...
async def get_service_cases(
    ...
    - 改进建议案例
    """
    cases = await insights_service.get_service_cases(db, current_user, case_type)
    return success(
...
async def get_competitor_opportunities(
    ...
    - 行动建议
    """
    opportunities = await insights_service.get_competitor_opportunities(db, current_user)
    return success(
